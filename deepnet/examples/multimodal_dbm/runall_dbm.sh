#!/bin/bash
# This script trains a Multimodal DBM using deepnet.
...
deepnet=/home/xcs/deepnet-master/deepnet
...
if ${clobber} || [ ! -e ${model_output_dir}/image_rbm1_LAST ]; then
  ...
  python ${trainer} models/image_rbm1.pbtxt \
    trainers/dbm/train_CD_image_layer1.pbtxt eval.pbtxt || exit 1
  python ${extract_rep} ${model_output_dir}/image_rbm1_LAST \
    trainers/dbm/train_CD_image_layer1.pbtxt image_hidden1 \
...
if ${clobber} || [ ! -e ${model_output_dir}/image_rbm2_LAST ]; then
  ...
  python ${trainer} models/image_rbm2.pbtxt \
    trainers/dbm/train_CD_image_layer2.pbtxt eval.pbtxt || exit 1
  python ${extract_rep} ${model_output_dir}/image_rbm2_LAST \
    trainers/dbm/train_CD_image_layer2.pbtxt image_hidden2 \
...
if ${clobber} || [ ! -e ${model_output_dir}/text_rbm1_LAST ]; then
  ...
  python ${trainer} models/text_rbm1.pbtxt \
    trainers/dbm/train_CD_text_layer1.pbtxt eval.pbtxt || exit 1
  python ${extract_rep} ${model_output_dir}/text_rbm1_LAST \
    trainers/dbm/train_CD_text_layer1.pbtxt text_hidden1 \
...
if ${clobber} || [ ! -e ${model_output_dir}/text_rbm2_LAST ]; then
  ...
  python ${trainer} models/text_rbm2.pbtxt \
    trainers/dbm/train_CD_text_layer2.pbtxt eval.pbtxt || exit 1
  python ${extract_rep} ${model_output_dir}/text_rbm2_LAST \
    trainers/dbm/train_CD_text_layer2.pbtxt text_hidden2 \
...
if ${clobber} || [ ! -e ${model_output_dir}/joint_rbm_LAST ]; then
  ...
  python ${trainer} models/joint_rbm.pbtxt \
    trainers/dbm/train_CD_joint_layer.pbtxt eval.pbtxt || exit 1
  python ${extract_rep} ${model_output_dir}/joint_rbm_LAST \
    trainers/dbm/train_CD_joint_layer.pbtxt joint_hidden \
    ${data_output_dir}/joint_rbm_LAST ${gpu_mem} ${main_mem} || exit 1
fi

#TRAIN DBM
if ${clobber} || [ ! -e ${model_output_dir}/multimodal_dbm_LAST ]; then
  echo "Training model DBM."
  python ${trainer} models/multimodal_dbm.pbtxt \
    trainers/dbm/train_CD_dbm.pbtxt eval.pbtxt || exit 1
  python extract_dbm_representation.py ${model_output_dir}/multimodal_dbm_LAST \
    trainers/dbm/train_CD_dbm.pbtxt ${data_output_dir}/dbm_LAST image_hidden1 image_hidden2 text_hidden1 text_hidden2 joint_hidden
fi
...
if ${clobber} || [ ! -e ${data_output_dir}/generated_text/data.pbtxt ]; then
  ...
  python sample_text.py ${model_output_dir}/multimodal_dbm_LAST \
    trainers/dbm/train_CD_dbm.pbtxt ${data_output_dir}/generated_text \
    ${prefix}/flickr_z.pbtxt ${gpu_mem} ${main_mem} || exit 1
fi

# COLLECT REPRESENTATIONS WITH/WITHOUT TEXT IN ONE DATA PBTXT.
if ${clobber} || [ ! -e ${data_output_dir}/dbm_all_layers/data.pbtxt ]; then
  echo "Collecting all representations"
  python collect_dbm_reps.py ${model_output_dir}/multimodal_dbm_LAST \
    ${data_output_dir}/dbm_all_layers \
    ${data_output_dir} \
    ${prefix} \
...
for i in `seq ${numsplits}`; do
  ...
  if ${clobber} || [ ! -e ${data_output_dir}/split_${i}/data.pbtxt ]; then
    python split_reps.py ${data_output_dir}/dbm_all_layers/data.pbtxt \
